...
description: Find property listings with WebSearch/WebFetch or claude-in-chrome tools, normalize them into Listing-shaped dicts, and run the house-hunting harness via the `house-hunt` MCP server. Use when a user wants real candidate properties ranked against a buyer brief.
metadata:
...
Use this skill when the user wants help finding actual properties to buy or rent and you have web or browser tools available in this Claude Code session.

This skill treats **web discovery** and **harness evaluation** as separate steps:

1. collect candidate listings from the web using WebSearch / WebFetch / claude-in-chrome
...
Use WebSearch to find candidate URLs, then WebFetch or claude-in-chrome to fetch the page content.
...
| Site blocks WebFetch or WebSearch | Try `claude-in-chrome` `navigate` + `read_page` if available, or ask the user to paste the listing. |
